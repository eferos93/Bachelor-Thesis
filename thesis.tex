...
Service-Oriented Computing (SOC) is the computing paradigm that utilizes services as fundamental elements for developing applications/solutions. To build the service model, SOC relies on the Service Oriented Architecture (SOA) \cite{Papazoglou03service-orientedcomputing:}. SOC abstracts from the implementation details of services by imposing a standard communication mechanism between the entities in an SOA \cite{Jol}.

\chapter{Jolie and the Language Server Protocol}
\label{sec:JolieLSP}
\section{Jolie: Java Orchestration Language Interpreter Engine}
From the perspective of the SOA, it is possible to identify a separation between \textit{behavioral} and \textit{architectural} composition of services. The first denotes a series of interactions to be performed in order to reach a goal, while the second deals with the topological structure of a SOA, managing its execution and integration. The lack of a homogeneous solution between the \textit{behavioral} and the \textit{architectural} composition of services require ad-hoc interventions in order to integrate them, which will make the entire architecture less modular and difficult to maintain.
...
\section{Future Work}
Future work include extending the Jolie library, refactoring the actual Language Server code and implement the \hyperref[distrArch]{distributed architecture}.

The first consists in integrate the Inspector service in order to be able to provide, together with information regarding the \textbf{deployment}, a data structure representing the \emph{Abstract Syntax Tree} of the program, from which, by extending the \verb|completion| operation, the Language Server will be able to provide also a list of possible completion items for \textbf{variables}. This can be done by interfacing the Inspector with \textbf{Jolie's front end} i.e. the Jolie parser. As a consequence, the LS can be extended in order to create a hierarchy of symbols to be sent to the development tool, granting the possibility to implement features like \textbf{go-to definition} and \textbf{find references}.

The second consists of refactor the actual searching algorithm, which is a simple \textit{linear search}, to a more sophisticated one, like the \textit{Approximate String Matching}, also referred as Fuzzy search. The general goal is to perform string matching of a pattern in a text where one or both of them have suffered some kind of (undesirable) corruption \cite{Navarro99aguided}. Another improvement can consists in generating a sorted list of operations from the Inspector response and then implement a Binary Search algorithm when generating the completion list to be sent to the client. However, this sorting would be applied every time an inspection is executed, namely every time a \verb|didOpen| or \verb|didChange| message is received. The second message is sent every time a single char is typed in the document. Therefore another improvement can be implementing a mechanism that does not let an update in the LS state for each \verb|didChange| message received. More in detail, when the LS starts a \emph{change bit} is set to 1 and a timer is set. Following the pseudo code:
\begin{algorithm}[H]
\caption{ChangeBit}
\begin{algorithmic}
\If {$changeBit == 1$}
...
\end{algorithmic}
\end{algorithm}
The change bit is set to 1 when the timer is elapsed. This will avoid useless computations by the LS. This can be implemented in both architectures: in the distributed one in the Proxy Service, while in the local in the Main Service.
