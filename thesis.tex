...
The server structure is the presented in \hyperref[sec:arch]{Chapter 4}, thus it will be composed by four Jolie programs together with the files with respectively the types definitions and the interfaces used by the various ports.
\subsection{Types}
Though Jolie operations can be type as \textcolor{color:keyword}{undefined}, I preferred define all the types basing on the LSP specification, in order to type check every message to be received and sent. Therefore, maintainability will be affected positively, as changes in the type definitions of LSP messages that might be applied in future releases of the protocol, can be identified immediately as if a type check fails, an error will sent to the invoker or 
\chapter{Validation}
