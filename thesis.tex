...
All the data types illustrated are language agnostic, meaning that can be applied to all the programming languages. This is due to the protocol simplicity: it is more simple to standardize a document URI or a position inside a it, that standardizing an \textit{Abstract Syntax Tree} and compiler symbol across different programming languages.\\The aforementioned simplicity is shown in the following JSON-RPC objects that refers to the completion request and response:\\
\textbf{Request}:
\begin{lstlisting}[language=json]
{
	"jsonrpc": "2.0",
