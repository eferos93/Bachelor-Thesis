...
Given a text document position, the server ought to compute a list of possible completion item to integrate what is already present the aforementioned position. We implemented this functionality for auto-completing output statements. For instance, if in the given text document position the server finds "pri", it ought to compute a list of output operations that are found in the interfaces of the outpurt ports of the service. In this case, the server will return the following list: \texttt{["print@Console( \$0 )( \$1 )", "println@Console( \$1 )( \$2 )"]}. These completion items are in the form of \textbf{snippets}. A \textbf{snippet} is a small and reusable piece of code, in which tab stops and placeholders can be defined with the \texttt{\$1}, \texttt{\$2} and \texttt{\$\{3:foo\}}. \texttt{\$0} identifies the end of the snippet. Placeholders with equal identifiers are linked, that is typing in one will update the others too \cite{LSPSpec}.\\Following the code of the \verb|completion| input operation.
\begin{lstlisting}[language=Jolie]
	/*
...
\section{Functional requirements}
\hyperref[FRVal]{Table 6.1} shows which functional requirements are met (\greenc), partially met (\orangec) and unmet (\redx).
\begin{table}[t]
	\caption{Functional Requirements}
	\begin{tabularx}{\textwidth}{c|c|X}
		\hline
...
FR 2 and 3 are fully met. See \hyperref[FR2Val]{figure 6.2} and \hyperref[FR2Val_res]{figure 6.3} for FR2, \hyperref[FR3Val]{figure 6.4} and \hyperref[FR3Val_res]{figure 6.5} for FR3.
\begin{figure}[t]
	\centering
...
\end{figure}
\subsection{FR 4}
This is managed by the Utils service. In order to verify the requirement, URIs of the actual documents saved in the server memory were printed in the console every time a \verb|didOpen| or \verb|didClose| message were received. \hyperref[FR4Val_1]{Figure 6.6} shows that after opening the third document, the Language Server prints the URIs of the documents in memory and when the file \emph{utils.iol} is closed, it prints the URIs of the remaining two, as shown in \hyperref[FR4Val_2]{figure 6.7}. Utils uses the \emph{Inspector} service for generating a data structure containing all the information regarding the \textbf{deployment}, and no information regarding the \textbf{behavior}.
\begin{figure}[t]
	\centering
...
\section{Non-Functional requirements}
The non-functional requirements defined at \hyperref[sec:NFR]{Chapter 3} are all met, except for NFR 4 (partial) and 5 (unmet), as \hyperref[NFRVal]{table 6.2} shows.
\begin{table}[t]
	\caption{Non-Functional Requirements}
	\label{NFRVal}
	\begin{tabularx}{\textwidth}{c|c|X}
		...
		\redx & NFR 5 & The server if distributed, must be able to handle more clients sending multiple requests at the same time.\\\hline
	\end{tabularx}
\end{table}
