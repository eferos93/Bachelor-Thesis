...
\acknowledgements
First of all I would like thank my thesis supervisor, Professor Marino Miculan, for the directions 
...
Service-Oriented Computing (SOC) is the computing paradigm that utilizes services as fundamental elements for developing applications/solutions. To build the service model, SOC relies on the Service Oriented Architecture (SOA) \cite{Papazoglou03service-orientedcomputing:} which is a software design technique, used to represent a distributed system based on services. A \emph{service} is an independent software entity that can be accessed remotely by other services through message passing. In the last decade a SOA variant emerged, called \emph{Microservices}, which %\emph{Microservices architecture}% are a modern interpretation of the SOA
is a software development technique where an application is a collection of loosely coupled, fine-grained services that communicate with lightweight protocols over well-defined APIs. In other words, this architecture model describes a particular way of designing software applications as suites of independently deployable services \cite{msFowler}. Microservices are getting more very popular over the years, with growing international communities, such as the Microservices Community \cite{msCommunity}, and multinational corporation providing platforms for developing those kind of applications, for instance Amazon AWS and Google Cloud provide platforms where users can deploy their microservice architectures.

SOC abstracts from the implementation details of services by imposing a standard communication mechanism between the entities in an SOA \cite{Jol}. From the perspective of these architectures, it is possible to identify a separation between \textit{behavioral} and \textit{architectural} composition of services. The first denotes a series of interactions to be performed in order to reach a goal, while the second deals with the topological structure of a SOA/microservices architecture, managing its execution and integration. The lack of a homogeneous solution between the \textit{behavioral} and the \textit{architectural} composition of services require ad-hoc interventions in order to integrate them, which will make the entire architecture less modular and difficult to maintain. \emph{Jolie} was created with the intention to fill this gap. This language permits to define services, their behavioral composition, supporting different communication technologies and their organization inside a microservices architecture.
...
This thesis is the result of a 4 months internship at the University of Southern Denmark, with the Concurrency and Logic research team \cite{C&L} under the supervision of Professor Fabrizio Montesi \cite{MontesiWP}, which is the Jolie project leader and maintainer, and its aim is to design and implement a software tool which provides these features to be integrated in commonly used editors, like Visual Studio Code and Atom.

Implementing such a tool, i.e. a Language Server (LS), can take a significant effort. Traditionally this work had to be repeated for each development tool, as each of them provides different APIs for implementing the same feature \cite{LSPIntro}. Therefore, the solution proposed is to develop a LS that supports the \emph{Language Server Protocol} (LSP) by Microsoft, which aim is to standardize the communication between an IDE and a LS. This solves the problem presented above: with a LS that supports the LSP, the first can be re-used in multiple development tools that support the protocol.
...
\end{lstlisting}
if type \verb|undefined| is passed for the \verb|Type| argument, the following JSON string is obtained
\begin{lstlisting}[language=json]
	{
		"node" : 1
	}
...
Capabilities like the \textbf{go-to definition} and \textbf{find references} were not possible to implement due to the lack of a tool for obtaining an Abstract Syntax Tree of the behavior. As a consequence, completion feature is supported only for \textbf{output ports} and \textbf{keywords}, while it is not possible to auto-complete variables. Regarding the architecture, only the local one presented in \hyperref[sec:arch]{Chapter 4} was implemented.
...
\end{algorithmic}
\end{algorithm}
This will avoid useless computations by the LS. This can be implemented in both architectures: in the distributed one in the Proxy Service, while in the local in the Main Service.
