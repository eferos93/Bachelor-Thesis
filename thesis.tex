...
Implementing advanced features like auto-complete, hover information, diagnostics and go to definition for a programming language can take a significant effort. Typically, this work has to be repeated for every editor or IDE as each of them might offer different APIs for implementing the same feature. A \emph{Language Server} (LS) is a software program that is intended to provide the aforementioned language-specific features and communicate with a development tool through a communication protocol.\\The \emph{Language Server Protocol} was developed by Microsoft to standardize the protocol for how such servers and IDEs communicate. Therefore, on one hand, the single LS can be reused for each development tool that implements the aforementioned protocol, on the other hand, the editor can support multiple languages with minimal effort.
...
A LS in order to work with LSP does not need to implement all the features. LSP provide \textbf{capabilities}. A capability groups a set of language features. The protocol permits the server to notify the tool which capabilities supports, therefore the tool adapts itself in order to send notifications and requests of the supported capabilities.
...
The LS is \emph{stateful}, meaning that it keeps track of the state of interaction. In particular, the state is kept in memory and it is \emph{transient}. This means that if for some reason, outside the normal termination behavior, the LS terminates its execution, the state is lost. This implies that, in the distributed model, the LS must be able to distinguish which client the information kept belongs to by using \emph{sessions}, which must be handled in parallel.
...
The main service presents two input ports and three output ports. One input port is used to receive the requests and notifications sent by the client, the other is used by an another service, called \textbf{utils}, with the purpose to receive all the notifications to be sent to the client through one of the output ports. The remaining two output ports, point to the two main services of the server, \textbf{textDocument} and \textbf{workspace} and they should be aggregated by the first mentioned input port, therefore every message is automatically forwarded to the respective service.

The operations offered by this service are those necessary for initialization and termination of the server. For more details see \cite{LSPSpec}. The service must handle at least the following requests and notifications:
\begin{itemize}
...
\begin{itemize}
	...
	\item \verb|textDocument/didOpen|, \verb|textDocument/didChange|, \verb|textDocument/didClose| notifications. Messages used for synchronize the document's truth with the language server. The service should forward the message to the service \textbf{utils} that will respectively store, update or delete the information regarding the document (URI, text, etc.);
...
\begin{enumerate}
	\item \textbf{Complying with the LSP's JSON-RPC specifications}: LSP uses a simplified version of JSON-RPC protocol, whereas Jolie internal JSON-RPC protocol uses it over HTTP. Therefore, the protocol was extended in order to be capable of handling also LSP messages. Moreover, the Jolie interpreter, when receiving a JSON-RPC message is able to redirect it to the correct operation by using the \emph{method} field in the JSON-RPC message. Despite this, most of the method names in LSP have prefixes, for instance \verb|textDocument/didOpen|, which is problematic as operation names in Jolie does not support special characters such as the "/". As a consequence, Jolie's protocol must be extended in order to alias the operation names properly. 
...
\end{figure}
and when the file \emph{utils.iol} is closed, it prints the URIs of the remaining two.
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{tests/screenshots/synch_2.png}
\end{figure}
Utils uses the \emph{Inspector} service for generating a data structure containing all the information regarding the \textbf{deployment}, and no information regarding the \textbf{behavior}.
\subsection{FR 5}
...
\begin{table}[H]
	...
	\end{tabularx}
\end{table}
The actual implementation of the server corresponds to the local architecture presented in \hyperref[sec:arch]{Chapter 4}, there was not enough time to develop and test the distributed one. Despite this, it should not be difficult to develop it, as it just require to deploy the local server implementation in a remote machine and create a \emph{Proxy Service} located in the clients machine, which stands in between of the development tool and the server.\\NFR 5 is a direct consequence of the previous requirements. However, setting the execution modality both of the proxy and of the server to \emph{concurrent}.
...
Capabilities like the \textbf{go-to definition} and \textbf{find references} were not possible to implement due to the lack of a tool for obtaining an Abstract Syntax Tree of the behavior, and there was not enough time in other to implement the aforementioned tool. As a consequence, completion feature is supported only for \textbf{output ports} and \textbf{keywords}, while it is not possible to auto-complete variables. Regarding the architecture, I was not able to implement the \textit{distributed} version of the Language Server due to the lack of time, as it would require a mechanism of handling sessions, by using \textit{Correlation Sets} \cite{Jol}.
